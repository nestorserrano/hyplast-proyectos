...
@section('content')
    <div class="card">
        <form action="{{ route('proyectos.tasks.update', $task->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required>
                                ...
                                <option value="training" {{ old('status', $task->status) == 'training' ? 'selected' : '' }}>Capacitación</option>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <p class="text-muted">Seleccione un departamento para ver sus usuarios y agregar/modificar asignaciones</p>
                    </div>
                </div>
            ...
            <div class="card-footer">
                ...
                <a href="{{ route('proyectos.tasks.show', $task->id) }}" class="btn btn-secondary">
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Cargar asignaciones existentes de la tarea
            @php
                $existingAssignmentsByDept = [];
                foreach($task->departmentAssignments as $deptAssignment) {
                    $deptCode = $deptAssignment->departamento_codigo;
                    $existingAssignmentsByDept[$deptCode] = [
                        'code' => $deptCode,
                        'users' => []
                    ];
                }

                // Agrupar usuarios asignados por departamento
                $usersGroupedByDept = [];
                foreach($task->assignedUsers as $user) {
                    // Si el usuario tiene departamento, agregarlo
                    // Por ahora, si no podemos determinar el departamento del usuario,
                    // agregamos todos los usuarios a todos los departamentos asignados
                    foreach(array_keys($existingAssignmentsByDept) as $deptCode) {
                        if (!isset($usersGroupedByDept[$deptCode])) {
                            $usersGroupedByDept[$deptCode] = [];
                        }
                        $usersGroupedByDept[$deptCode][] = [
                            'id' => $user->id,
                            'name' => $user->name,
            ...
            // Cargar asignaciones existentes desde el servidor
            let existingAssignments = @json($existingAssignmentsByDept);
            let usersGrouped = @json($usersGroupedByDept);

            // Cargar cada departamento con sus usuarios
            Object.keys(existingAssignments).forEach(function(deptCode) {
                // Necesitamos hacer una petición para obtener el nombre del departamento
                $.ajax({
                    ...
                    success: function(response) {
                        let dept = response.departments.find(d => d.code === deptCode);
                        if (dept && usersGrouped[deptCode]) {
                            assignments.push({
                                code: deptCode,
                                name: dept.name + ' (' + deptCode + ')',
                                allUsers: false,
                                users: usersGrouped[deptCode] || []
                            });
                        }
                    }
                });
            ...
            // No validar en edit - permitir guardar sin cambios en asignaciones
        });
    </script>
@stop
